...
You should start to see reasonable images after ~5 epochs, and good images by ~15 epochs.
You should use a GPU, as the convolution-heavy operations are very slow on the CPU.
Prefer the TensorFlow backend if you plan on iterating, as the compilation time can
be a blocker using Theano.
Timings:
...
Consult https://github.com/lukedeo/keras-acgan for more information and example output
"""
...
def build_generator(latent_size):
    ...
    cls = Flatten()(Embedding(num_classes, latent_size, embeddings_initializer='glorot_normal')
                    (image_class))
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs + 1):
        ...
        # we don't want the discriminator to also maximize the classification accuracy of
        # the auxiliary classifier on generated images, so we don't train discriminator to
        # produce class labels for generated images (see https://openreview.net/forum?id=rJXTf9Bxg).
        ...
        for index in range(num_batches):
            ...
            # We reshape the sampled labels to be (batch_size, 1) so that we can feed them
            # into the embedding layer as a length one sequence
            ...
            # make new noise. we generate 2 * batch size here such that we have the generator
            # optimize over an identical number of images as the discriminator
        ...
        sampled_labels = np.array([[i] * num_rows for i in range(num_classes)]
                                  ).reshape(-1, 1)
        ...
        img = np.concatenate((generated_images, np.repeat(np.ones_like(x_train[:1]),
                                                          num_rows, axis=0), real_images))
